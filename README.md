...
옹벽류는 현장 진입로가 좁아 25톤 트럭을 못 쓰고 5톤단축·5톤장축 트럭으로만 배차하는 경우가 많아, 다른 파트와 별도로 계산합니다. 자동 최적화가 아니라 **담당자가 직접 5톤단축/장축 대수를 선택**하면 그 배차 기준으로 운반비를 보여주는 방식입니다(현장마다 실제로 부를 수 있는 차량 종류가 다르기 때문).

1. 물량(㎡)을 입력하고, 5톤단축 대수 / 5톤장축 대수를 직접 입력합니다. 입력하는 동안 선택한 배차로 실을 수 있는 최대 ㎡가 바로 표시됩니다.
2. 계산하기를 누르면 선택한 배차 그대로 운반비(5톤 요율 × 단축 대수 + 5톤축 요율 × 장축 대수)를 계산합니다. 선택한 배차가 물량보다 부족하면 경고가 뜨고, 그래도 계산 결과(비용)는 보여줍니다.
3. 참고용으로 "물량을 다 싣는 최저비용 조합"도 함께 안내합니다(선택한 배차와 다를 때만 표시). 강제 적용되지는 않으며 어디까지나 참고입니다.
